...
def build_env_map(environ: Dict[str, str], infix: str) -> Dict[str, Any]:
    ...
    layer_output = Path(environ[f"{prefix}layer_output"])
    _, layer_feature_json = environ[f"{prefix}layer_feature_json"].split()
    target_map = {
        **make_target_path_map(environ[f"{prefix}target_path_pairs"].split()),
        **make_target_path_map(environ[f"{prefix}builders"].split()),
    }
